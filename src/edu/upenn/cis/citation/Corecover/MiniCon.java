...
class MiniCon {
  ...
  static HashSet formMCDs(Query query, Vector<Single_view> views) {
    UserLib.myprintln("calling formMCDs()...");
    HashSet mcds = new HashSet();

    for (int i = 0; i < views.size(); i ++) {
      Single_view view = (Single_view) views.elementAt(i);
  ...
  // generates all MCDs for a query and views
  static HashSet formMCDs(Query query, Single_view view) {
