...
//          System.out.println("valid_tuple::" + tuple);
  ...
  public static HashSet computeViewTuples(Database canDb, Single_view view){//Vector<Subgoal> subgoals, HashMap<String, String> subgoal_name_mappings, String view_name, HashMap<String, Vector<Lambda_term>> subgoal_lambda_term_mappings ) {
    HashSet viewTuples = new HashSet();
...
//
  ...
  static boolean set_tuple_lambda_term(Tuple tuple, Query view)
  {
	  ...
	  for(int i = 0; i<view.lambda_term.size(); i++)
	  {
		  ...
		  String curr_table_name = curr_lambda_name.substring(0, curr_lambda_name.indexOf(init.separator));
...
//	  System.out.println(tuple.cluster_non_mapping_condition_ids);
  ...
  static void cluster_relational_subgoals(Single_view view, Tuple tuple, Conditions condition, int j, boolean get_mapping1, boolean get_mapping2)
  {
...
//        System.out.println(get_mapping2);
  ...
  static void computeTupleCore(Tuple viewTuple, Query query) {
    ...
    Single_view   view         = viewTuple.getQuery();
...
//  
  ...
  static boolean removeable(HashSet core, Mapping phi, HashMap reversedMap,
			    Single_view view, Argument viewArg, 
